...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            in_channels = output_filters[-1] if output_filters else int(hyperparams["channels"])
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        # Output : Batch_Size * Num_Anchors * (4+1+Num_classes) * Num_Grids * Num_Grids
        ...
        grid_x = torch.arange(num_Grids).repeat(num_Grids, 1).view([1, 1, num_Grids, num_Grids]).type(FloatTensor)
        grid_y = torch.arange(num_Grids).repeat(num_Grids, 1).t().view([1, 1, num_Grids, num_Grids]).type(FloatTensor)
